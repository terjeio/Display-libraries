...
bool setSysTickCallback (void (*fn)(void))
{
    ...
    driver.delayms(1); // start systick timer
    return true; // for now
}
...
//TODO: inform touch driver about changes!
	...
	driver.changeOrientation(orientation);
}
...
void clearScreen (bool blackWhite)
{
	...
	driver.setArea(0, 0, driver.display.Width - 1, driver.display.Height - 1);
    driver.writePixel(bgColor, (uint32_t)driver.display.Width * (uint32_t)driver.display.Height);
}
	...
	if(width) {
		...
		if(!paintSpace || opaque) {
			...
			while(fontColumn--) {
				...
				if(opaque)
					driver.setArea(x, displayRow, x, y);
...
//#define SLOWFPLOT
                    ...
                    while(fontRow--) {
                        ...
                        if(pixels & 0x01) {
                            if(!opaque)
                                driver.setArea(x, displayRow, x, displayRow);
                            driver.writePixel(fgColor, 1);
                        } else if(opaque)
                            driver.writePixel(bgColor, 1);
                        pixels >>= 1;
                        displayRow++;
					...
					while(fontRow--) {
					    ...
					    if((pixels & 0x01) != paint || !fontRow) {
                            if(paint) {
                                if(!opaque)
                                    driver.setArea(x, displayRow, x, displayRow + count - 1);
                                driver.writePixel(fgColor, count);
                            } else if(opaque)
                                driver.writePixel(bgColor, count);
                            displayRow += count;
                            count = 1;
                            paint = pixels & 0x01;
					    } else
				    ...
				    driver.writePixel(bgColor, fontRow);
				x++;
			}
		}
	}
...
void drawLine (uint16_t xStart, uint16_t yStart, uint16_t xEnd, uint16_t yEnd)
{
	if (yStart == yEnd) { // check if horizontal
		...
		driver.setArea(xStart, yStart, xEnd, yStart);
		driver.writePixel(fgColor, xEnd - xStart + 1);
		...
		driver.setArea(xStart, yStart, xStart, yEnd);
		driver.writePixel(fgColor, yEnd - yStart + 1);
	} else { // angled
...
{
    driver.setArea(xStart, yStart, xEnd, yEnd);
    driver.writePixel(fgColor, (uint32_t)(xEnd - xStart + 1) * (uint32_t)(yEnd - yStart + 1));
}
...
// TODO: implement clipping
    ...
    driver.setArea(ifp.x + rect->left, ifp.y + rect->top, ifp.x + rect->right, ifp.y + rect->bottom);
    driver.writePixels((uint16_t *)bitmap, (rect->right - rect->left + 1) * (rect->bottom - rect->top + 1));
...
void drawImageMono (uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data)
{
    ...
    driver.setArea(x, y, x + w - 1, y + h - 1);
